...
#if defined(NDEBUG)
# error "Poins cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x51a024a30b3f562f8db35db60a8d8115301f619b5955cb248b1fc6315035ba67");
//0x12a765e31ffd4059bada1e25190f6e98c99d9714d334efa41a195a7e7e04bfe2
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Poins: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Poins Signed Message:\n";
...
{
    // Poins: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
        if(nHeight == 1){
	nSubsidy = 30000100 * COIN;
}
    // Subsidy is cut in half every 600.000 blocks, which will occur approximately every 6 years
    nSubsidy >>= (nHeight / 600000); // Poins: 600k blocks in ~6 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Poins: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Poins: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Poins: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Poins: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
	...
	pchMessageStart[3] = 0x3f;
        hashGenesisBlock = uint256("0xb421cc8d3cb60840afc50a562a98b421777aaaf7e8e59f8bb6eeed180c2034ef");
    }

    //
    // Load block index from databases 0xf5ae71e26c74beacc88382716aced69cddf3dffff24f384e1808905e0188f68f
    //
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Indonesia Times 24/August/2018, Poins Crypto, PIN 18";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1535069294;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084926038;

        if (fTestNet)
        {
            block.nTime    = 1535069207;
            block.nNonce   = 387223446;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9e30b1cbcdc41906ebc40fec94d47e5fb84c1aaa409b3c8ab68704ff3c046864"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe6, 0x26, 0x17, 0x0d }; // Poins: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PoinsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PoinsMiner(CWallet *pwallet)
{
    printf("PoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("poins-miner");
    ...
    try { loop {
        ...
        printf("Running PoinsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PoinsMiner, pwallet));
}
